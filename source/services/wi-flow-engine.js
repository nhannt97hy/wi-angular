...
angular.module(moduleName, []).factory('wiFlowEngine', function (wiApiService, wiComponentService, wiPetrophysics) {
  ...
  const transformFn = (fn) => {
    return (element, context, next) => {
      ...
      //   return arg;
      // });
      wiApiService.getTask(idTask, (task, err) => {
        if (err) return next(err);
        fn({taskConfig: task.content, name: task.name}, (results) => next(null));
      });
    };
  };

  const getServices = (fns) => {
    const transformedFns = {};
    const fnNames = Object.keys(fns);
    fnNames.forEach((key) => {
      transformedFns[key] = transformFn(fns[key]);
    });
    return transformedFns;
  };

  async function execute({ diagram }) {
    ...
    return new Promise((resolve, reject) => {
      engine.execute(
        {
          // listener: listener,
          services: getServices(wiPetrophysics),
        },
        (err, definition) => {
          if (err) {
            reject(err);
