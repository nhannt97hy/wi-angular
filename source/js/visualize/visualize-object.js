...
/* THANG
ObjectOfTrack.prototype.showObjectOfTrack = function(showObjectOfTrack){
    if(showObjectOfTrack) {
        this.objectContainer
            .selectAll("div:not(.vi-histogram-tooltip)")
            .style('opacity', 1);
    } else {
        this.objectContainer
            .selectAll('div:not(.vi-histogram-tooltip)')
...
/* THANG 
ObjectOfTrack.prototype.horizontalLineDragCallback = function(line) {
    this.showObjectOfTrack(false);
    let self = this;
    ...
    let newYs = [];
    this.horizontalLines.each(function() {
    ...
    this.placeHolder
        .attr('width', self.getViewportX()[1] - self.getViewportX()[0])
        .attr('height', maxY - minY);
    this.placeHolderRect
        .attr('x', self.getViewportX()[0])
        .attr('y', minY)
        .attr('width', self.getViewportX()[1] - self.getViewportX()[0])
        .attr('height', maxY - minY);
    this.placeHolderText
        .attr('x', self.getViewportX()[1] / 2)
        .attr('y', (y == maxY ? y-10:y+10))
        .text("[" + parseFloat(self.startDepth).toFixed(2) + " - " + parseFloat(self.endDepth).toFixed(2) + "]");
}

ObjectOfTrack.prototype.onHorizontalLineDragEnd = function (wiApiService) {
    let self = this;
    let y = d3.mouse(this.root.node())[1];
    let transformY = this.getTransformY();
    ...
    if(this.viHistogram) {
        if(this.viHistogram.histogramModel.properties.intervalDepthTop == this.startDepth &&
            this.viHistogram.histogramModel.properties.intervalDepthBottom == this.endDepth) {
            return;
        }
        this.viHistogram.histogramModel.properties.intervalDepthTop = this.startDepth;
        this.viHistogram.histogramModel.properties.intervalDepthBottom = this.endDepth;
        this.viHistogram.startDepth = this.startDepth;
        this.viHistogram.endDepth = this.endDepth;
    }
    this.onViewportChange(null, null, minY, maxY, false, false);
    wiApiService.editObject(this.exportsProperties(), function() {
        console.log("Object edited");
    })
}
*/