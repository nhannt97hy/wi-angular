...
function Controller(apiService, $timeout, $element){
    ...
    socket.on('sendMessage', function (data) {
        ...
        $timeout(function(){
            ...
            textMessage.scrollTop(textMessage.prop("scrollHeight"));
        });
    });
}
...
function Controller(apiService, $timeout){
    ...
    socket.on('send-member-online', function(data) {
        console.log(data);
...
function Controller(apiService, $http, $timeout) {
        let self = this;
        this.show = false;
        this.LProject = {};
        this.conver = {};
        this.token = '';
        this.inChatTab = true;
        this.inHelpDeskTab = false;
        this.newChat = 0;
        this.newHelpDesk = 0;
        this.sumNew = 0;
        $('.tab-chat').addClass('active');
        $('#tab-chat').addClass('in active');
        function getListUser(LProject, cb) {
                apiService.getListUserOfProject({
                        project_name: LProject.name,
                        owner: LProject.owner?LProject.owner:localStorage.username
                }, self.token, (res) => {
                        if (res) {
                                self.listUser = res;
        ...
        function getChatGroup(LProject) {
                $('.helpDesk').removeClass('active');
                $('#tab-helpdesk').removeClass('in active');
                $('.members').removeClass('active');
                $('#tab-members').removeClass('in active');
                $('.tab-chat').addClass('active');
                $('#tab-chat').addClass('in active');
                self.inChatTab = true;
                self.inHelpDeskTab = false;
        ...
        this.initChat = function(token, LProject) {
                console.log('initChat()', token, LProject);
                if(!token)
                        toastr.error('Authentization fail');
                else {
                        self.token = token;
                        if(LProject) {
                                if(self.LProject.name != LProject.name) {
                                        if(self.conver.id)
                                                socket.emit('off-project', {idConversation: self.conver.id, username: self.user.username});
                                        self.LProject = LProject;
                                        getListUser(LProject, function() {
                                                if(self.listUser.length>=2) {
                                                        self.showChatGroup = true;
                                                        getChatGroup(LProject);
                                                } else{
                                                        self.showChatGroup = false;
                                                        $('.helpDesk').addClass('active');
                                                        $('#tab-helpdesk').addClass('in active');
                                                }
                                        });
                                }
                        } else {
