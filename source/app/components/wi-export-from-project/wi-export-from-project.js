const componentName = 'wiExportFromProject';
const moduleName = 'wi-export-from-project';
...
function Controller($scope, $timeout, $attrs, wiApiService, wiComponentService, wiOnlineInvService) {
    ...
    this.$onInit = function () {
        wiComponentService.putComponent('wiExportFromProject', self);
        ...
        wiApiService.listWells({ idProject: projectLoaded.idProject }, function (wells) {
            ...
            if (wells) {
                ...
                for (well of cutWells) {
                    self.projectConfig.push(utils.createWellModel(well));
                }
            }
        })
    ...
    function selectHandler(currentNode, noLoadData, rootNode, callback) {
        ...
        if (currentNode.type == 'well' && !noLoadData) {
            if (rootNode === self.projectConfig) {
    ...
    this.onSelectButtonClicked = function () {
        ...
        if (Array.isArray(self.projectConfig && self.projectConfig.__SELECTED_NODES)) {
            ...
            for (child of self.projectConfig.__SELECTED_NODES) {
                console.log('selectedNode', self.projectConfig.__SELECTED_NODES);
                ...
                if (parentDataset) {
                    ...
                    if (wellExisted) {
                        ...
                        $timeout(function () {
                            ...
                            // self.idExportQueueItems.push(getIdObjectFromNode(wellNodeCopy, self.projectConfig));
                        })
                        break;
                    }
...
app.component(componentName, {
    templateUrl: 'wi-export-from-project.html',
