...
module.exports = function (ModalService, zoneTrackProperties, callback) {
    function ModalController($scope, wiComponentService, wiApiService, close) {
        ...
        this.foreground = function () {
            ...
            dialogUtils.colorPickerDialog(ModalService, self.fill.foreground, function (colorStr) {
                self.fill.pattern.foreground = colorStr;
            });
        }
        this.background = function () {
            dialogUtils.colorPickerDialog(ModalService, self.fill.background, function (colorStr) {
                self.fill.pattern.background = colorStr;
            });
        }
        this.choosePattern = function(name) {
            dialogUtils.fillPatternDialog(ModalService, name, self.fill.foreground, self.fill.background, function(_name) {
                if(_name) {
                    // self.fill.pattern.name = _name;
                    self.fill.pattern = _name;
                }
            });
        }
